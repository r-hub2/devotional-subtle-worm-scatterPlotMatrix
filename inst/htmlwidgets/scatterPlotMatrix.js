(() => {
  ...
  var BrushSlider = class {
    ...
    update() {
      ...
      const dx = this.xOriented ? ScatterPlotMatrix.margin.l : ScatterPlotMatrix.margin.l + ScatterPlotMatrix.margin.r + this.scatterPlotMatrix.width - 16;
    ...
    buildChainGroup() {
      ...
      const chainGroup = select_default2(this.scatterPlotMatrix.bindto + " .MultiPlot svg").append("g").attr("class", "chainGroup").attr("transform", `translate(${ScatterPlotMatrix.margin.l + ScatterPlotMatrix.margin.r + this.scatterPlotMatrix.width - 16}, ${ScatterPlotMatrix.margin.t / 4})`).on("mousedown", function() {
    ...
    brushBehavior() {
      return this.buildBrushBehavior().handleSize(4).extent(
        this.xOriented ? [
          [0, -5],
          [this.scatterPlotMatrix.width, 5]
        ] : [
          [-5, 0],
          [5, this.scatterPlotMatrix.height]
        ]
      ).on("brush", (event) => {
  ...
  var ScatterPlotMatrix = class _ScatterPlotMatrix {
    ...
    resize(width, height) {
      this.setSize(width, height);
      select_default2(this.bindto + " .MultiPlot svg").attr("width", this.width + _ScatterPlotMatrix.margin.l + _ScatterPlotMatrix.margin.r).attr("height", this.height + _ScatterPlotMatrix.margin.b + _ScatterPlotMatrix.margin.t);
      this.removePlots();
    ...
    buildMainDomElements(config) {
      ...
      const mspDiv = select_default2(this.bindto).append("div").attr("class", "mspDiv").classed("withWidgets", controlWidgets).classed("withoutWidgets", !controlWidgets);
    ...
    appendPlotSvg() {
      const mspSvg = select_default2(this.bindto + " .MultiPlot").append("svg").attr("width", this.width + _ScatterPlotMatrix.margin.l + _ScatterPlotMatrix.margin.r).attr("height", this.height + _ScatterPlotMatrix.margin.b + _ScatterPlotMatrix.margin.t);
    ...
    appendSvgDefs() {
      ...
      defs.append("clipPath").attr("id", "tile-clip").append("rect");
    }
    appendBrushSliders() {
    ...
    updatePlots(updateType) {
      ...
      if (updateType & ScatterPlot.INIT) {
        ...
        diagPlot.append("g").attr("class", "spArea").attr("clip-path", "url(#tile-clip)");
        ...
        scatterPlot.append("g").attr("class", "spArea").attr("clip-path", "url(#tile-clip)");
